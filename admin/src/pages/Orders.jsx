      ...
      <div>
        {
          orders.map((order, index) => (
            <div className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_2fr_1fr_1fr_1fr] gap-3 items-start border-2 border-gray-200 p-5 md:p-8 my-3 md:my-4 text-xs sm:text-sm text-gray-700' key={index}>
              ...
              <div>
                <p className='text-sm sm:text-[15px]'>Số lượng sản phẩm: {order.items.length}</p>
                <p className='mt-3'>Phương thức thanh toán: {order.paymentMethod}</p>
                <p>Tình trạng thanh toán : {order.payment ? 'Done' : 'Pending'}</p>
                <p>Ngày tạo đơn: {new Date(order.date).toLocaleDateString()}</p>
              </div>
